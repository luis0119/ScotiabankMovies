...
class SaveMoviesUseCaseTest {
    ...
    fun setUp() {
        ...
        saveMoviesUseCase = SaveMoviesUseCase(movieFactory,movieService,Dispatchers.Main)
    }
    ...
    fun testGetMoviesWhenCategoryFound() = testCoroutineRule.runBlockingTest{
        ...
        //Act
        saveMoviesUseCase.execute(movieRequest,movieResponse)
        //Assert
        coVerify {
    ...
    fun testGetMoviesWhenCategoryNotFound() = testCoroutineRule.runBlockingTest{
        //Arrange
        val movieRequest = MovieRequestBuilder(position = 0).build()
        val movieResponse = MovieResponseBuilder().build()
